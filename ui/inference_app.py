        ...
        if st.button("🚀 Run Prediction", type="primary", use_container_width=True, key="run_prediction"):
            with st.spinner("Generating forecast..."):
                ...
                if results['success']:
                    ...
                    # Note: predictions_df contains only future forecasts, no historical data
                    historical_mask = pd.Series([False] * len(predictions_df), index=predictions_df.index)
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        # Prepare download data - predictions_df already contains only forecast data
