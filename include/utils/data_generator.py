...
class RealisticSalesDataGenerator:
    ...
    def generate_promotions(self) -> pd.DataFrame:
        """Generate random promotions for products"""
        promotions = []
        major_events = [
            ("Black_Friday", 11, 4, 5, 25), # 4th Friday of November, 5 days, 25% off
            ...
            ("New Year Sale", 1, 1, 7, 0.20), # 1st January, 7 days, 20% off
            ("Presidents Day", 2, 15, 3, 0.15), # 15th February, 3 days, 15% off
            ("Memorial Day Sale", 5, 30, 3, 0.20), # Last Monday of May, 3 days, 20% off
            ("Independence Day Sale", 8, 15, 3, 0.20), # 15th August, 3 days, 20% off
            ("July 4th Sale", 7, 4, 3, 0.20), # 4th July, 3 days, 20% off
            ("Labor Day Sale", 9, 1, 3, 0.15), # First Monday of September, 3 days, 15% off
        ...
        while current_date <= self.end_date:
            ...
            for event_name, month, day, duration, discount in major_events:
                if event_name == "Black Friday":
                    ...
                    event_date = thursdays[3] + timedelta(days=1) # 4th Thursday of November
                else:
                    try:
                ...
                if self.start_date <= event_date <= self.end_date:
                    for d in range(duration):
                        ...
                        if promo_date <= self.end_date:
                            promo_products = random.sample(list(self.all_products.keys()), random.randint(5, 15))
        ...
        n_flash_sales = int((self.end_date - self.start_date).days * 0.05) # 5% of the time
        flash_dates = pd.date_range(self.start_date, self.end_date, periods=n_flash_sales)

        for date in flash_dates:
            promo_products = random.sample(list(self.all_products.keys()), random.randint(3, 8))
            for product_id in promo_products:
                promotions.append({
                    "product_id": product_id,
                    "date": date,
                    "discount_percent": random.uniform(0.1, 0.3), # 10% to 30% discount
    ...
    def generate_sales_data(self, output_dir: str = "/tmp/sales_data") -> Dict[str, List[str]]:
        ...
        while current_date <= self.end_date:
            ...
            logger.info("Generating sales data for {}".format(date_string))
            ...
            for store_id, store_info in self.stores.items():
                ...
                holiday_factor = 1.3 if is_holiday else 0.0
                ...
                if store_events_df.empty:
                    ...
                    if not event.empty:
                        store_event_impact = event.iloc[0]["impact"] + 1.0

                # Calculate store traffic
                store_traffic = int(
                    base_traffic * dow_factor *
                    store_event_impact * weather_factor *
                    holiday_factor * np.random.normal(1.0, 0.05)
                )
                ...
                for product_id, product_info in self.all_products.items():
                    ...
                    price_factor = 1.0 / (1.0 + product_info["price"] / 100.0) # higher price, lower volume
        ...
        metadata_in = {
            ...
            "total_files_generated": sum(len(v) for v in file_paths.values()),
        }
        ...
        logger.info(f"Generated sales data for {len(file_paths['sales'])} days")
