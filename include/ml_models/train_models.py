...
from include.ml_models import ensemble_model
...
class ModelTrainer:
    ...
    def prepare_data(self, df: pd.DataFrame, target_col: str = "sales",
                     ...
                     category_cols: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        df_features = self.feature_engineer.create_all_features(
            ...
            group_cols=group_cols, categorical_cols=category_cols
        )
        ...
        validation_size = int(len(df_sorted) * self.training_config["validation_size"])

        train_df = df_sorted[:train_size]
        val_df = df_sorted[train_size: train_size+validation_size]
        test_df = df_sorted[train_size+validation_size:]
    ...
    def train_all_models(self, train_df: pd.DataFrame, val_df: pd.DataFrame,
        ...
        try:
            ...
            ensemble_models = EnsembleModel(ensemble_models, ensemble_weight)

            self.models["ensemble"] = ensemble_models

            ensemble_metrics = self.calculate_metrics(y_test, ensemble_pred)

            self.mlflow_manager.log_metrics({f"Ensemble {k}": v for k, v in ensemble_metrics.items()})
            self.mlflow_manager.log_model(ensemble_models, "Ensemble", input_example=X_train.iloc[:5])
            ...
            try:
                ...
                logger.error(f"Error during visualisation generation: {viz_error}")
            ...
            try:
                ...
                logger.info("Artifacts synced successfully to S3")
                ...
                if not verification_results["success"]:
                    logger.error(f"S3 Artifact verification failed. Please check the logs for details.")
